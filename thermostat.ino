...
const int relayPin = 5;	    // use this pin to drive the transistor
...
void loop()                    
{
  float sensor, voltage, degreesC, degreesF;

  sensor = analogRead(0);

  if (relayIsOn)
  {
    // adjust voltage to account for relay load
    sensor -= 4;
  }

  voltage = sensor * 0.004882814;
